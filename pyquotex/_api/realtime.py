...
class RealtimeMixin:
    ...
    async def start_candles_all_size_stream(self, asset: str) -> bool:
        ...
        try:
            if hasattr(self.api, "subscribe_all_size"):
                await self.api.subscribe_all_size(self.codes_asset.get(asset, asset))
        except Exception as e:
            logger.error(
                '**error** start_candles_all_size_stream subscribe: %s', e
            )
            await self.connect()
        while await self.check_connect():
            if self.api is None:
            ...
            if time.time() - start > 20:
                logger.error(
                    f'**error** fail {asset} '
                    'start_candles_all_size_stream late for 20 sec'
                )
                return False
            try:
    ...
    async def start_mood_stream(
        ...
        if hasattr(self.api, "subscribe_Traders_mood"):
            await self.api.subscribe_Traders_mood(self.codes_asset.get(asset, asset), instrument)
        # Give the server a short chance to emit the first sentiment frame,
        # but do not loop-spam subscribe messages.
        for _ in range(25):
            if self.api is None:
                return
            asset_code = self.codes_asset.get(asset, asset)
            if self.api.traders_mood.get(asset_code) or self.api.traders_mood.get(asset):
                return
            await asyncio.sleep(0.2)
