...
def _is_rate_limit_reason(reason: object) -> bool:
    text = str(reason or "").lower()
    return "429" in text or "rate limit" in text or "rate-limited" in text or "rate limited" in text


def _should_refresh_session(reason: object) -> bool:
    """Refresh HTTP login only for auth/session failures, not network/rate limits."""
    text = str(reason or "").lower()
    if _is_rate_limit_reason(text):
        return False
    auth_markers = (
        "authorization", "ssid", "token", "not authenticated", "auth", "rejected"
    )
...
class AccountMixin:
    ...
    async def connect(self) -> tuple[bool, str]:
        ...
        if not check or not await self.check_connect():
            # HTTP 429 is a broker/IP rate limit. Repeating HTTP login here makes
            # the situation worse, so stop cleanly and let the caller wait.
            ...
            if not _should_refresh_session(reason):
                try:
                    await self.api.close()
                except Exception:
                    pass
                return False, reason or "Websocket connection failed."

            logger.warning(
                "Initial websocket auth failed (%s). Refreshing session and retrying once.",
                reason,
            )
            self.session_data["token"] = None
            self.session_data["cookies"] = None
            update_session(self.email, self.session_data, self.host)
            try:
                await self.api.close()
            except Exception:
                pass

            # Rebuild the API to avoid stale cookies/headers/tasks, then perform
            # one clean HTTP login + WebSocket auth attempt.
            self.api = QuotexAPI(
                self.host,
                self.email,
                self.password,
            ...
            auth_check, auth_reason = await self.api.authenticate()
            if not auth_check:
            ...
            if _is_rate_limit_reason(reason):
                ...
                return False, reason
            if not check or not await self.check_connect():
                return False, reason or "Websocket connection rejected."
