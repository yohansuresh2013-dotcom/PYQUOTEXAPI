...
class QuotexAPI:
    ...
    def __init__(
        ...
        self.wss_url = (
            wss_url_override
            if wss_url_override
            else f"wss://ws2.{self.host}/socket.io/?EIO=3&transport=websocket"
        )
        ...
        self.browser = Browser(cookie_domain=self.host, proxies=proxies)
    ...
    async def _h_auth_ok(self, data: Any) -> None:
        ...
        self.slots.auth_status.set(True)
    ...
    async def _h_candle_generated(self, data: Any) -> None:
        ...
        if asset and period:
            asset_s = str(asset)
            period_i = int(period)
            self.candle_generated_check[asset_s][period_i] = data
            self.candle_generated_all_size_check[asset_s] = data
            await self.event_registry.set_event(f"candle_generated_{asset_s}_{period_i}", data)
            await self.event_registry.set_event(f"candle_generated_{asset_s}", data)
    ...
    async def _on_message(self, msg: bytes | str) -> None:
        ...
        try:
            ...
            if self.trace_ws and self.state.auth_status != AuthStatus.AUTHENTICATED:
                logger.debug("Received while not authenticated: %s", msg_str[:200])

            if "authorization/reject" in msg_str:
                logger.debug("Websocket authorization rejected: %s", msg_str)
                self.state.websocket_error_reason = "Websocket authorization rejected."
                self.state.auth_status = AuthStatus.FAILED
                if not self.slots.auth_status.is_set():
                    self.slots.auth_status.set(False)
                await self.event_registry.set_event(
                    "auth_changed", self.state.auth_status
                )
                return
            elif "s_authorization" in msg_str:
                logger.debug("Websocket authorization success")
                self.state.auth_status = AuthStatus.AUTHENTICATED
                self.state.status = WebsocketStatus.CONNECTED
                if not self.slots.auth_status.is_set():
                    self.slots.auth_status.set(True)
    ...
    def _on_error(self, error: Exception | str) -> None:
        ...
        """
        msg = str(error)
        if "429" in msg and "rate" not in msg.lower():
            msg = "WebSocket rate limited by broker (HTTP 429). Cool down before reconnecting."
        logger.error(msg)
        self.state.websocket_error_reason = msg
    ...
    async def start_websocket(self) -> tuple[bool, str]:
        ...
        """
        if getattr(self.state, "is_rate_limited", False):
            remaining = int(self.state.rate_limit_remaining)
            return False, (
                f"WebSocket rate limited by broker (HTTP 429). "
                f"Wait about {remaining}s before reconnecting."
            )
        ...
        if not self.state.SSID:
            auth_ok, auth_reason = await self.authenticate()
            if not auth_ok:
        ...
        extra_headers = {
            ...
            "Referer": f"{self.https_url}/{self.lang}/trade",
            "Cookie": self.session_data.get("cookies") or "",
    ...
    async def send_ssid(self) -> bool:
        """Sends the SSID token and waits briefly for authorization."""
        if not self.state.SSID:
            return False
        self.state.auth_status = AuthStatus.AUTHENTICATING
        self.slots.auth_status.clear()
        await self.ssid(self.state.SSID)
        try:
            await self.slots.auth_status.wait(timeout=5)
    ...
    async def connect(self, is_demo: bool) -> tuple[bool, str]:
        ...
        if ok:
            auth_ok = await self.send_ssid()
            if not auth_ok:
                return False, self.state.websocket_error_reason or "Websocket authorization timeout."
        return ok, reason
