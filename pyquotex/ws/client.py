...
def _extract_http_status(exc: BaseException) -> int | None:
    """Best-effort status extraction across websockets versions."""
    ...
    if isinstance(status_code, int):
        return status_code
    text = str(exc)
    m = __import__("re").search(r"HTTP\s+(\d{3})", text, __import__("re").I)
    if m:
        return int(m.group(1))
    return None


def _format_ws_error(exc: BaseException) -> str:
    status = _extract_http_status(exc)
    if status == 429:
        return "WebSocket rate limited by broker (HTTP 429)."
    if status:
        return f"WebSocket handshake rejected by broker (HTTP {status})."
    return str(exc)
...
class WebsocketClient:
    ...
    async def run_forever(
        ...
        while True:
            try:
                ...
                reason = _format_ws_error(e)
                if status == 429:
                    cooldown = float(getattr(self.policy, "rate_limit_cooldown", 75.0) or 75.0)
                    self.api.state.rate_limited_until = time.monotonic() + cooldown
                    reason = (
                        f"{reason} Cooldown {int(cooldown)}s before next connect; "
                        "do not loop login/connect."
                    )
                    logger.warning(reason)
                    self.api._on_error(reason)
                    if getattr(self.policy, "stop_on_rate_limit", True):
                        return
                else:
            ...
            # If the server explicitly rate-limited us, never hammer it.
            if getattr(self.api.state, "is_rate_limited", False):
                remaining = self.api.state.rate_limit_remaining
                if getattr(self.policy, "stop_on_rate_limit", True):
                    return
    ...
    async def _connect_once(
        ...
        # websockets >=14 uses additional_headers; older versions use
        # extra_headers. Pick the right kwarg at runtime for compatibility.
        sig = getattr(websockets.connect, "__signature__", None)
        try:
            import inspect as _inspect
            params = _inspect.signature(websockets.connect).parameters
        except Exception:
            params = {}
